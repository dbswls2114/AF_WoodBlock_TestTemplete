...
public class GameManager : MonoBehaviour
{
    ...
    // 추후 prefab을 직접 Inspector에서 할당하지않고 Resources 폴더에서 로드 혹은 초기화 시 생성,탐색하는 방식으로 변경 예정
    ...
    public GameObject gameOverUI; // 게임 오버 UI 패널 [UiManager로 이전 예정]
    public TextMeshProUGUI scoreText; // 점수 텍스트 (TMP)  [UiManager로 이전 예정]
    public TextMeshProUGUI bestscoreText; // 점수 텍스트 (TMP) [UiManager로 이전 예정]
    ...
    void HandleInput() //[입력 처리] 블록 드래그 앤 드롭 [Inputmanager로 이전 예정]
    {
    ...
    void CreateBoardVisuals() // [기능] 보드 격자 시각적 요소 생성
    {
    ...
    void ShowPreview(Block block) // [기능] 블록 놓기 미리보기
    {
    ...
    void ClearPreview() // [기능] 블록 놓기 미리보기 초기화
    {
    ...
    void HighlightLinesToClear(List<Vector2Int> newBlockCoords) // [기능] 놓을 블록으로 인해 제거될 줄 하이라이트
    {
    ...
    void SpawnNewBlocks() // [기능] 핸드 블록 3개 생성
    {
    ...
    void TryPlaceBlock(Block block) // [검사] 블록 놓기 시도
    {
        bool canPlace = true;
    ...
    bool IsValidCoord(Vector2Int pos) // [검사] 격자 좌표가 보드 범위 내에 있는지 확인
    {
        return pos.x >= 0 && pos.x < boardSize && pos.y >= 0 && pos.y < boardSize;
    }

    void PlaceBlockOnGrid(Block block, List<Vector2Int> coords) // [기능] 블록을 격자에 놓기
    {
        SoundManager.Instance.PlaySFX(SoundManager.Instance.placeBlockClip);
    ...
    void GetLinesToClear(int[,] targetGrid, out List<int> fullRows, out List<int> fullCols) // [검사] 채워진 줄 찾기
    {
    ...
    void CheckLines() // [기능] 채워진 줄 확인 및 제거
    {
    ...
    private void ClearLines(List<int> rows, List<int> cols) // [검사] 채워진 줄 확인 및 제거
    {
    ...
    void AddScore(int amount) // [기능] 점수 추가 및 최고 기록 갱신
    {
        currentScore += amount;
    ...
    void UpdateScoreUI(bool bestscore = false) // [기능] 점수 UI 갱신 [UIManager로 이전 예정]
    {
    ...
    // --- 유틸리티 함수 ---
    Vector3 GetObjectWorldSize(Transform t) // [툴] 오브젝트의 월드 크기 반환
    {
    ...
    Vector2Int WorldToGrid(Vector3 worldPos) // [툴] 월드 좌표를 격자 좌표로 변환
    {
    ...
    Vector3 GridToWorld(int x, int y) // [툴] 격자 좌표를 월드 좌표로 변환
    {
        float boardRealWidth = cellSize * boardSize;
    ...
    // --- 게임 오버 확인 ---
    void CheckGameOver() // [검사] 게임 오버 확인
    {
    ...
    private bool CanFitAnywhere(Block block) // [검사] 블록이 보드 어디든 들어갈 수 있는지 확인
    {
    ...
    private bool CheckFitAt(int startX, int startY, int[,] shape) // [검사] 특정 위치에 블록이 들어갈 수 있는지 확인
    {
