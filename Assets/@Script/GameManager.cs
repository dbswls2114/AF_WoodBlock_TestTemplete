...
public class GameManager : MonoBehaviour
{
    ...
    void SpawnNewBlocks()
    {
        ...
        float startX = -handAreaSize.x / 2.0f + slotWidth / 2.0f; // 첫 슬롯 중앙 X좌표
        ...
        for (int i = 0; i < 3; i++)
        {
            ...
            blockObj.transform.position = new Vector3(startX + (i * slotWidth), handCenterPos.y, 0);
    ...
    {
        Vector3 firstPiecePos = block.pieces[0].position; // 첫 번째 조각(혹은 기준점)의 위치
        Vector2Int gridPos = WorldToGrid(firstPiecePos);
        ...
        else
        {
            ...
            // 실패 시 원위치
            block.ReturnToOrigin(originalBlockPos, previewScale);
        }
    }
    Vector2Int WorldToGrid(Vector3 worldPos)
    {
        float offset = (boardSize * cellSize) / 2.0f - (cellSize / 2.0f);
    ...
    void PlaceBlockOnGrid(Block block, List<Vector2Int> coords)
    {
        ...
        for (int i = 0; i < coords.Count; i++)
        {
            ...
            // 블록 조각을 그리드 시스템으로 이동 (부모 변경 X, 위치만 고정하거나 새로 생성)
            // 여기서는 블록 오브젝트의 조각을 떼어내서 정리
            Transform piece = block.pieces[i];
            piece.position = GridToWorld(pos.x, pos.y);
            piece.SetParent(null); // 블록 부모에서 분리
            gridVisuals[pos.x, pos.y] = piece;
        }

        // 블록 오브젝트 제거 및 리스트에서 삭제
        activeBlocks.Remove(block);
        Destroy(block.gameObject);

        AddScore(10 + coords.Count); // 기본 배치 점수
    ...
    void CheckLines()
    {
        ...
        {
            bool full = true;
            for (int x = 0; x < boardSize; x++) if (gridData[x, y] == 0) full = false;
        ...
        {
            bool full = true;
            for (int y = 0; y < boardSize; y++) if (gridData[x, y] == 0) full = false;
    ...
    void ClearLines(List<int> rows, List<int> cols)
    {
        ...
        foreach (int x in cols)
        {
            for (int y = 0; y < boardSize; y++)
            {
                if (gridData[x, y] == 1) // 이미 row에서 지워졌을 수 있음
                {
                    gridData[x, y] = 0;
                    if (gridVisuals[x, y] != null) piecesToDestroy.Add(gridVisuals[x, y]);
                    gridVisuals[x, y] = null;
